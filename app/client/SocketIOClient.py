...
class SocketIOClient:
    ...
    def emit_file_added():
        ...
        def run_client_no_port():
            print('Emitting files added (no port)')
            from socketIO_client import SocketIO
            socketIO = SocketIO('localhost', os.getenv('PORT'))
            socketIO.emit('files added', broadcast=True)
            print('Emitted! (no port)')
            socketIO.disconnect()
        ...
        thread_test_no_port = threading.Thread(target=run_client_no_port)
        thread_test_no_port.start()
